from flask import  session,g,flash, Flask, render_template, redirect, url_for, g
    ...
    if form.validate_on_submit():
        ...
        if user:
            if check_password_hash(user.password, form.password.data):
                ...
                current_user_path = os.path.join(current_path, "user_data", str(user.id))

                if os.path.exists(os.path.join(current_user_path, "safequestion.json")) == False:
...
def signup():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            #g = new_user.id
            session['userid'] = str(new_user.id)
            session['type'] = "setsafequestion"
...
def safequestion():
    ...
    if form.validate_on_submit():
        if 'type' in session and session['type'] == 'setsafequestion':
            global user_folder
            ...
            with open(os.path.join(current_path, "user_data", session['userid'], "safequestion.json"), "w") as file:
                file.write(json.dumps(answer))

            with open(os.path.join(current_path, "user_data", session['userid'], "info.json"), "w") as file:
            ...
            return """<h2>You do not have access to this page yet</h2>"""
    else:
...
def changepassword():
    ...
    if form.validate_on_submit():
        if 'type' in session and session['type'] == 'changepassword':
